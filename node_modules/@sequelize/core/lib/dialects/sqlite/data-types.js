...
class JSON extends BaseTypes.JSON {
  parseDatabaseValue(value) {
    ...
    if (typeof value !== "string") {
      throw new Error(
        `DataTypes.JSON received a non-string value from the database, which it cannot parse: ${import_node_util.default.inspect(value)}.`
      );
    }
    try {
      ...
      throw new import_errors.BaseError(
        `DataTypes.JSON received a value from the database that it not valid JSON: ${import_node_util.default.inspect(value)}.`,
        { cause: error }
      );
    }
  }
